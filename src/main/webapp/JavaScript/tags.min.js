...
class Tags {
    ...
    buildSuggestions() {
        let e = this.selectElement.querySelectorAll("option");
        for (let t = 0; t < e.length; t++) {
            let s = e[t];
            if (!s.getAttribute("value")) continue;
            let l = document.createElement("li"), i = document.createElement("a");
            l.append(i), i.classList.add("dropdown-item"), i.setAttribute(VALUE_ATTRIBUTE, s.getAttribute("value")), i.setAttribute("href", "#"), i.innerText = s.innerText, this.dropElement.appendChild(l), i.addEventListener("mouseenter", e => {
                this.removeActiveSelection(), l.querySelector("a").classList.add(ACTIVE_CLASS)
            }), i.addEventListener("click", e => {
                e.preventDefault(), this.addItem(i.innerText, i.getAttribute(VALUE_ATTRIBUTE)), this.resetSearchInput(), this.hideSuggestions()
            })
        }
    }
    ...
    addItem(e, t) {
        ...
        if (tagsLength <= 5) {
            if(e !== '' && e !== ' '){
				...
				//'<img src="IMG/delete.svg" alt="delete icon"> </img>';
				//var click = "hallowelt()";
